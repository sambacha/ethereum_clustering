...
def data_pipeline(df):
    ...
    # build pipeline
    pipe = Pipeline([("log", log), ("scale", scale), ("PCA", pca)])
...
def find_category_of_cluster(cl, dflabel, category="Exchange"):
    ...
    for clust in np.unique(cl.labels_):
        ...
        if num > num_of_type:
            ...
            type_cluster = clust
        print(
            "cluster number   {}   number of type found: {}    cluster size: {}   label density: {}".format(
                clust, num, size_of_cluster, density
            )
        )
    return type_cluster
...
def plot_tsne_with_labels(tsne_results, df, dflabel, categs, colors):
    ...
    # categories
    cats = np.array(
        [cat(addr, labeled_addresses, dflabel) for addr in df["address"]]
    )  # [address_mask] ]) #added address mask for all clusters
    ...
    for c in list(dflabel["category"].unique()):
        ...
        if c in categs:
            ...
            plt.scatter(
                tsne_results[(labelmask & catmask)][:, 0],
                tsne_results[(labelmask & catmask)][:, 1],
                s=100,
                c=color,
                alpha=1,
                label=lbl,
...
def recluster(df, cl, clusters, n_clusters):
    ...
    kclusterer = KMeansClusterer(
        n_clusters, distance=nltk.cluster.util.cosine_distance, repeats=50
    )
