...
def get_predictions(df, dflabel, target):
    ...
    dflabel[target] = [
        1 if dflabel["category"][i] == target else 0 for i in range(len(dflabel))
    ]
...
def plot_predictions(tsne_results, predictions):
    ...
    plt.scatter(
        ...
        c="green",
        alpha=0.4,
        label=("Predicted exchanges"),
    )

    plt.scatter(
        tsne_results[~mask][:, 0], tsne_results[~mask][:, 1], c="gray", s=20, alpha=0.1
    )
